...
# 内置 models.dev 权威基准标准库 (保证离线开箱即用)
...
class ModelsDevSyncService:
    ...
    async def init_default_models(self):
        ...
        async with AsyncSessionLocal() as session:
            for item in DEFAULT_MODELS_CATALOG:
    ...
    async def sync_from_models_dev(self) -> Dict[str, Any]:
        """尝试从 models.dev 线上 API 同步最新模型与价格"""
        try:
                ...
                if resp.status_code == 200:
                    ...
                    async with AsyncSessionLocal() as session:
                        for m in data if isinstance(data, list) else data.get("models", []):
                            ...
                            exist = res.scalar_one_or_none()
                            if exist:
                                exist.official_input_price = m.get("pricing", {}).get("prompt", exist.official_input_price)
                                exist.official_output_price = m.get("pricing", {}).get("completion", exist.official_output_price)
                                exist.updated_at = datetime.utcnow()
                                updated_count += 1
                        await session.commit()
                    self.last_sync_time = datetime.utcnow()
                    return {"status": "success", "source": "online", "updated_count": updated_count}
        except Exception as e:
            print(f"[ModelsDevSync] Online fetch failed, fallback to local dataset: {e}")
        
        # 离线回退同步
        await self.init_default_models()
        return {"status": "success", "source": "local_fallback", "updated_count": len(DEFAULT_MODELS_CATALOG)}
