...
class RelayFetcherService:
    ...
    async def detect_and_sync_site(self, site_id: int) -> Dict[str, Any]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            # 阿里百炼专属自动爬取同步
            if site.site_type == "aliyun_bailian":
                try:
                    ...
                    scrape_res = await bailian_scraper.scrape_pricing_page()
                    if scrape_res.status == "success" and scrape_res.models:
                        import_res = await bailian_scraper.save_to_database(
                            models=scrape_res.models,
                            usd_to_cny_rate=rate,
                            site_id=site.id
                        )
                        ...
                        return {
                            ...
                            "discovered_count": len(scrape_res.models),
                            "matched_count": import_res.total_imported,
                            "message": f"成功同步阿里百炼 {import_res.total_imported} 款模型官方定价"
                        }
                except Exception as e:
                    print(f"[RelayFetcher] Bailian scraper sync error: {e}")
